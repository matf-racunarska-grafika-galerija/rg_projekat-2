...
struct ProgramState {
    ...
    glm::vec3 vecCalibrate = glm::vec3(0.0f); //pomocna promenjiva koja mi sluzi za podesavanje preko imguia
    glm::vec3 vecRotate = glm::vec3(0.0f);//pomocna promenjiva koja mi sluzi za podesavanje preko imguia
    glm::vec3 lightPos = glm::vec3(1.2f, 1.0f, 2.0f);

    float fineCalibrate = 0.001f;//pomocna promenjiva koja mi sluzi za podesavanje preko imguia
    glm::vec3 dirLightDir = glm::vec3(-0.2f, -1.0f, -0.3f);//potencijalno nepotrebno
    glm::vec3 dirLightAmbDiffSpec = glm::vec3(0.3f, 0.3f,0.2f);//potencijalno nepotrebno
...
int main() {
    ...
    Shader asteroidShader("resources/shaders/rock.vs", "resources/shaders/rock.fs");
    //skybox
    ...
    // load models
    // -----------
    Model ourModel("resources/objects/proba/D77-TC_Pelican_v014.obj");
    ourModel.SetShaderTextureNamePrefix("material.");

    //planeta
    Model planetaModel("resources/objects/planet/Beta_Hydri_8192.obj");
    planetaModel.SetShaderTextureNamePrefix("material.");

    //rock
    Model rockModel("resources/objects/rock/rock.obj");
    rockModel.SetShaderTextureNamePrefix("material.");

    //list of trasnformation matrices
    ...
    for (unsigned int i = 0; i < 2; i++)
    {
        ...
        // checks whether framebuffer is complete
        if (glCheckFramebufferStatus(GL_FRAMEBUFFER) != GL_FRAMEBUFFER_COMPLETE)
            std::cout << "Framebuffer not complete!" << std::endl;
    }
    ...
    //activation of buffers for skybox
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        ourShader.use();
        ourShader.setBool("opacity", programState->opacity);

        pointLight.position = glm::vec3(5.0 * cos(currentFrame), 4.0f, 5.0 * sin(currentFrame));
        ourShader.setVec3("pointLight.position", programState->lightPos);
        ...
        ourShader.setFloat("p", programState->p);
...
//        boxModel.Draw(ourShader);
        ...
        model = glm::rotate(model,glm::radians(currentFrame*programState->fineCalibrate), glm::vec3(0.0f ,1.0f, 0.0f));
        ...
        modelPlaneta = glm::translate(modelPlaneta,glm::vec3(0.0f,-10.0f,0.0f));
        modelPlaneta = glm::scale(modelPlaneta, glm::vec3(10.0f));
//        modelPlaneta = glm::scale(modelPlaneta, glm::vec3(15.0f));

        ourShader.setMat4("model", modelPlaneta);
        planetaModel.Draw(ourShader);
        glDisable(GL_CULL_FACE);
        ...
        model = glm::mat4(1.0f);
        model = glm::translate(model, programState->lightPos);
...
void processInput(GLFWwindow *window) {
    ...
    if(glfwGetKey(window, GLFW_KEY_RIGHT) == GLFW_PRESS)
        programState->lightPos.x -= 0.09;
    if(glfwGetKey(window, GLFW_KEY_LEFT) == GLFW_PRESS)
        programState->lightPos.x += 0.09;
    if(glfwGetKey(window, GLFW_KEY_P) == GLFW_PRESS)
        programState->lightPos.z -= 0.09;
    ...
    if(glfwGetKey(window, GLFW_KEY_Q) == GLFW_PRESS){
        programState->p -= 0.05;
        programState->p = max(0.0f, programState->p);
        cout << programState->p << '\n';
    }

    if(glfwGetKey(window, GLFW_KEY_E) == GLFW_PRESS){
        programState->p += 0.05;
        programState->p = min(1.0f, programState->p);
        cout << programState->p << '\n';
    }
}
...
void key_callback(GLFWwindow *window, int key, int scancode, int action, int mods) {
    ...
    if(key == GLFW_KEY_M and action == GLFW_PRESS){
        programState->flip_M = !programState->flip_M;
        programState->CameraMouseMovementUpdateEnabled = programState->flip_M;
    }
    if(key == GLFW_KEY_B and action == GLFW_PRESS){
        programState->flip_B = !programState->flip_B;
        programState->opacity = programState->flip_B;
    }
}
